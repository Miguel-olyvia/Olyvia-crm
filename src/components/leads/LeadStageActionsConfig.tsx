...
import {
  Plus, Trash2, UserCheck, ClipboardList, ArrowRightLeft,
...
const ACTION_LABELS: Record<string, { label: string; icon: React.ReactNode; description: string }> = {
  convert_to_contact: {
    label: "Converter para Contacto",
    icon: <UserCheck className="w-4 h-4 text-emerald-600" />,
    description: "Converte a lead em contacto automaticamente",
  },
  convert_to_client: {
  ...
  change_field: {
    ...
    description: "Altera um campo da lead automaticamente",
  },
};

export function LeadStageActionsConfig({ stages, companyId }: Props) {
  const { toast } = useToast();
  const [actions, setActions] = useState<StageAction[]>([]);
  const [loading, setLoading] = useState(true);
  const [allStagesMap, setAllStagesMap] = useState<Record<string, { label: string; color: string }>>({});
  ...
  useEffect(() => {
    ...
    stages.forEach(s => { map[s.id] = { label: s.label || s.name, color: s.color }; });
    (async () => {
      ...
      (data || []).forEach((s: any) => {
        if (!map[s.id]) map[s.id] = { label: s.label || s.name, color: s.color };
      });
      setAllStagesMap(map);
    })();
  }, [stages]);
  ...
  const handleAdd = async () => {
    ...
    const { error } = await (supabase.from("lead_stage_actions") as any).insert([{
  ...
  const getStageName = (stageId: string) => {
    return allStagesMap[stageId]?.label || stageId;
  };

  const getStageColor = (stageId: string) => {
    return allStagesMap[stageId]?.color || "#6b7280";
  };
