-- Orçamentos — persistir item_supplier_id (referência de fornecedor concreta) nas
-- linhas gravadas por rpc_save_quote() — Fase 2 do plano de fornecedores multi-stock.
-- Origem: C:\Olyvia\vault\plano-referencia-fornecedor-orcamentos.md, secção 1.3
-- Forward-only migration. Do not fold into the baseline. Do not edit an already-applied migration.
--
-- Why this migration exists
-- -------------------------
-- 20261112140000_quote_lines_item_supplier_reference.sql adicionou a coluna
-- quote_lines.item_supplier_id, mas rpc_save_quote() (a única via de escrita em
-- quote_lines a partir do handleSave() do QuoteBuilder) ainda não a inclui em nenhum
-- dos dois pontos onde faz INSERT INTO public.quote_lines — o das linhas principais
-- (secção "2. quote_lines: insert the full computed set") e o das linhas de cada
-- inline quote (secção "7. inline quotes"). Sem esta migration, qualquer
-- item_supplier_id enviado pelo frontend seria silenciosamente descartado.
--
-- Supersedes: 20260813010000_quotes_audit_bypass_child_trigger_fix.sql (confirmado
-- por `grep -rl "FUNCTION public.rpc_save_quote" supabase/migrations/` — é a definição
-- mais recente; nenhuma migration posterior a redefine). Corpo COPIADO byte-a-byte
-- dessa migration, incluindo todos os comentários internos, EXCETO as duas alterações
-- cirúrgicas descritas abaixo. Nenhuma outra lógica, assinatura ou comportamento foi
-- alterado.
--
-- What changed (exactly two inserts touched, nothing else)
-- ----------------------------------------------------------
--   1. INSERT INTO quote_lines da secção "2. quote_lines: insert the full computed
--      set" (linhas principais, p_lines): coluna `item_supplier_id` adicionada à
--      lista de colunas logo depois de `bundle_id`, e o valor
--      `nullif(v_line ->> 'item_supplier_id', '')::uuid` adicionado à lista de VALUES
--      na posição correspondente (logo depois do valor de bundle_id).
--   2. INSERT INTO quote_lines do loop de inline quotes na secção "7. inline quotes"
--      (p_inline_quotes): mesma alteração — `item_supplier_id` adicionado depois de
--      `bundle_id` nas colunas, e `nullif(v_iq_line ->> 'item_supplier_id', '')::uuid`
--      depois do valor de bundle_id nos VALUES.
--
-- Retrocompatibilidade: item_supplier_id é opcional em ambos os pontos — payloads sem
-- essa chave (frontend antigo em cache, ou linhas sem nenhum item_suppliers associado)
-- resolvem `nullif(... ->> 'item_supplier_id', '')::uuid` para NULL sem erro, exatamente
-- como já acontece hoje com bundle_id/catalog_item_id/product_id/service_id. Nenhum
-- payload existente deixa de funcionar.
--
-- Prerequisites:
--   20260813010000_quotes_audit_bypass_child_trigger_fix.sql    — definição atual (mais
--                                                                  recente) de rpc_save_quote()
--   20261112140000_quote_lines_item_supplier_reference.sql      — coluna
--                                                                  quote_lines.item_supplier_id
--                                                                  tem de existir antes desta
--                                                                  função tentar escrevê-la
...
-- ============================================================
--
-- 1. item_supplier_id agora presente nas duas listas de colunas/valores de
--    INSERT INTO quote_lines (linhas principais e inline quotes):
--
--      SELECT pg_get_functiondef('public.rpc_save_quote(uuid, jsonb, jsonb, jsonb, jsonb, jsonb)'::regprocedure)
--        LIKE '%item_supplier_id%';
--      -- Esperado: true.
--
-- 2. Payload sem 'item_supplier_id' em nenhuma linha continua a gravar
--    quote_lines.item_supplier_id = NULL, sem erro (retrocompatibilidade com FE
--    antigo em cache).
--
-- 3. Comportamento inalterado em tudo o resto: FIX 1 (qt > 0 em inline quotes),
--    FIX 2 (categoria = '' em inline quotes), auditoria consolidada num único
--    entity_audit_log por chamada (+1 por inline quote criada), autorização via
--    fn_deal_org_in_scope() — nenhum destes pontos foi tocado nesta migration.
