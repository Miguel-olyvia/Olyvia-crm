-- ============================================================
-- rpc_bulk_import_products(p_org_id uuid, p_products jsonb) — per-row error isolation
-- ============================================================
-- FIX: the previous version of this function (20260826010000) ran every row of
-- the incoming chunk inside the function's single implicit transaction and
-- RAISEd EXCEPTION on the first invalid row (bad org, missing id for update,
-- invalid mode, not-found product, etc). Since a chunk can carry up to 200
-- products, one malformed row rolled back ALL other valid rows in the same
-- chunk, and the frontend only ever saw one generic error for the whole file.
--
-- This migration replaces the function body so EACH row is executed inside
-- its own SAVEPOINT: on failure we ROLLBACK TO SAVEPOINT (undoing only that
-- row's partial writes) and record a failure entry instead of aborting the
-- whole batch. Rows that succeed keep their single fn_manual_audit_log call
-- exactly as before (bug-fix goal unchanged: one product write = one audit
-- row). The function's return shape gains a `status` field ('ok' | 'error')
-- and an `error` message field per row so the frontend can surface exactly
-- which SKU/line failed and why, and still commit every valid row in the
-- same chunk.
--
-- Returns: jsonb array of, for each input row:
--   success: { input_index, status: 'ok', product_id, action }
--   failure: { input_index, status: 'error', sku, error }
-- ============================================================
