-- Encomenda Cliente manual passa a gravar a morada de entrega.
-- Única alteração face à versão viva (pg_get_functiondef, 2026-09-24): o
-- orçamento sintético recebe quotes.obra_endereco a partir de
-- p_order ->> 'delivery_address' (vazio/espaços => NULL). Permissões,
-- validações, SECURITY DEFINER, search_path e fluxo draft→signed inalterados.
-- CREATE OR REPLACE mantém o dono, os GRANTs (anon, authenticated,
-- service_role, postgres: EXECUTE) e o COMMENT existentes.
