-- ==============================================================================
-- Atribui as 3 permissoes novas de processamento salarial (periodo.view,
-- periodo.gerir, lancamentos.gerir) ao papel super_admin.
--
-- POR APLICAR.
--
--
-- -- O PROBLEMA ----------------------------------------------------------------
--
-- 20261201220000 e 20261201230000 criaram 3 codigos novos e, de proposito,
-- nao os atribuiram a papel nenhum. Sem esta migracao o separador "Visao
-- geral" de /rh/vencimento existe na base e e inalcancavel -- nem o
-- super_admin o abre. Mesmo padrao de 20261201210000 (vencimento) e
-- 20261201120000 (admissao).
--
--
-- -- A REGRA NOVA --------------------------------------------------------------
--
-- O super_admin recebe os 3 codigos, incluindo os dois marcados perigosos
-- (periodo.gerir, lancamentos.gerir). Nenhum outro papel recebe nada -- RH
-- continua a precisar de atribuicao explicita no ecra de Papeis.
--
--
-- -- COMO SE REVERTE -----------------------------------------------------------
--
...
--    WHERE permission_code LIKE 'hr.processamento.%'
...
--
-- Prerequisitos:
--   20261201220000  catalogo hr.processamento.periodo.*
--   20261201230000  catalogo hr.processamento.lancamentos.gerir
-- ==============================================================================
...
BEGIN
  SELECT count(*) INTO v_catalogo
   ...
   WHERE code LIKE 'hr.processamento.%';

  IF v_catalogo <> 3 THEN
    RAISE EXCEPTION 'Esperavam-se 3 codigos hr.processamento.%%, encontraram-se %. Aplicar 20261201220000 e 20261201230000 primeiro.', v_catalogo;
  END IF;
...
SELECT r.id, p.code
 ...
 WHERE r.code = 'super_admin'
   AND p.code LIKE 'hr.processamento.%'
...
BEGIN
  ...
  RAISE NOTICE 'super_admin ficou com os % codigos de processamento em cada papel super_admin existente. Nenhum outro papel recebeu nada.', v_atribuido;
END;
$conferir$;
