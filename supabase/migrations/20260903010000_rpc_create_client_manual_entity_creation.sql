-- rpc_create_client_manual — extend to optionally create the entity itself.
-- 2026-09-03 | Module: Clients
-- Forward-only migration. Do not fold into the baseline. Do not edit an already-applied migration.
--
-- ============================================================
-- BUG — Client creation: entity fields invisible in the audit trail
-- ============================================================
-- rpc_create_client_manual (20260902010000) REQUIRES p_entity_id to already exist
-- ('Entidade obrigatória para criar cliente' when null). src/pages/AnewClients.tsx calls
-- createEntityWithIdentity() FIRST (raw, unaudited multi-table write: anew_entities,
-- anew_entity_emails, anew_entity_phones, fiscal_entities, anew_entity_fiscal_entities),
-- THEN calls this RPC with the already-existing entity_id. The single resulting audit row
-- only covers anew_clients/anew_entity_roles/anew_addresses — display_name/email/phone/vat
-- are invisible, because they were written by a call this RPC never sees.
--
-- Fix: mirror create_contact_with_role's (20260902010000) entity-creation branch exactly.
-- When p_entity_id IS NULL, this RPC now creates anew_entities / anew_entity_emails /
-- anew_entity_phones / fiscal_entities / anew_entity_fiscal_entities itself, folding their
-- diffs into the SAME consolidated audit row as the client/role/address writes. When
-- p_entity_id IS PROVIDED, behavior is byte-for-byte unchanged from 20260902010000 — the
-- new parameters are simply ignored on that path (they are only consulted inside the
-- p_entity_id IS NULL branch).
...
-- Old 11-arg signature is superseded by this 18-arg overload (7 new trailing
-- DEFAULT NULL params) — drop it so PostgREST/PostgreSQL don't have to disambiguate
-- two overloads with the same leading parameters.
