-- quote_lines.source_deal_need_id — a chave de origem ESTÁVEL da importação.
-- Forward-only migration. Do not fold into the baseline. Do not edit an already-applied migration.
--
-- Why this migration exists
-- --------------------------
-- 20261203030000 acrescentou source_deal_need_item_id para (a) idempotência do botão
-- de importar e (b) rastreabilidade. Faz (b), mas NÃO faz (a): fn_apply_deal_need()
-- apaga e reinsere deal_need_items a cada gravação da necessidade, pelo que os ids dos
-- itens mudam. Bastava alguém reabrir e gravar a necessidade para o orçamento deixar
-- de reconhecer o que já tinha importado — e a segunda importação duplicava as linhas.
--
-- deal_needs.id não tem esse problema: a necessidade é atualizada no lugar (UPDATE),
-- nunca recriada. É por isso a chave certa para responder a "esta necessidade já foi
-- importada para este orçamento?".
--
-- Divisão de responsabilidades entre as duas colunas, para não haver dúvidas:
--   · source_deal_need_id      → idempotência da importação. Estável. É esta que se
--                                consulta antes de importar.
--   · source_deal_need_item_id → rastreabilidade fina (que item concreto deu origem a
--                                que linha) DENTRO da mesma gravação da necessidade.
--                                Deixa de ser fiável assim que a necessidade é
--                                regravada — não usar para decisões.
--
-- Sem FOREIGN KEY, pela mesma razão documentada em 20261203030000: referência fraca e
-- deliberada, para a rastreabilidade sobreviver ao ciclo de vida da necessidade.
--
-- Parte 2 (rpc_save_quote): partiu-se da versão escrita em 20261203030000 (que já é
-- mais recente do que a definição viva na BD, ainda por aplicar). Assinatura de 7
-- parâmetros INALTERADA. Única alteração: source_deal_need_id na lista de colunas e no
-- VALUES do INSERT das quote_lines principais, ao lado de source_deal_need_item_id. O
-- INSERT das linhas de p_inline_quotes continua intacto.
--
-- Prerequisites:
--   20261203030000_quote_lines_source_deal_need_item_id.sql — source_deal_need_item_id
--                                                             e a versão de
--                                                             rpc_save_quote aqui
--                                                             estendida

-- ============================================================
-- 1. quote_lines — coluna de origem estável
-- ============================================================
...
COMMENT ON COLUMN public.quote_lines.source_deal_need_id IS
  'deal_needs.id da necessidade de onde esta linha foi importada. CHAVE DE IDEMPOTÊNCIA da importação: é esta que se consulta para saber se uma necessidade já foi importada para o orçamento. Estável — a necessidade é atualizada no lugar, nunca recriada. Referência FRACA (sem FK) de propósito. Não confundir com source_deal_need_item_id, que é volátil (fn_apply_deal_need faz delete+reinsert dos itens) e serve só para rastreabilidade fina dentro da mesma gravação.';
...
-- ============================================================
-- 2. rpc_save_quote — gravar também a origem estável
-- ============================================================
