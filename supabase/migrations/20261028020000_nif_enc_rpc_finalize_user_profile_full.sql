-- NIF Encryption — Phase 2, dual-write for rpc_finalize_user_profile_full
-- Forward-only migration. Do not fold into the baseline.
--
-- Same intent as 20261028010000_nif_enc_rpc_update_user.sql, applied to
-- rpc_finalize_user_profile_full: adds three OPTIONAL trailing parameters
-- (p_nif_encrypted, p_nif_hash, p_nif_tokens) so callers that already have
-- the encrypted NIF material (produced by
-- supabase/functions/_shared/nifCrypto.ts) can dual-write it into
-- public.fiscal_entities.nif_encrypted / nif_hash and
-- public.fiscal_entity_nif_tokens (added in
-- 20261023010000_fiscal_entities_nif_encryption_targets.sql) alongside the
-- existing plaintext fiscal_entities.nif INSERT.
--
...
-- (20260923010000_fix_finalize_user_profile_super_admin_escalation.sql).
-- When the three new parameters are omitted (or NULL), behavior is
-- unchanged: this function only ever INSERTs a fiscal_entities row (never
-- UPDATEs an existing one), so nif_encrypted/nif_hash are simply inserted as
-- NULL when not supplied, and the token-sync block is skipped entirely when
-- p_nif_tokens is NULL.
--
-- Prerequisites:
--   20260923010000_fix_finalize_user_profile_super_admin_escalation.sql —
...
BEGIN
  ...
  IF p_fiscal IS NOT NULL AND p_fiscal ? 'nif' THEN
    ...
    -- persist them on the same freshly-inserted row. This function only ever
    -- INSERTs a fiscal_entities row here (never updates an existing one), so
    -- there is no pre-existing value to protect against clobbering; omitted
    -- parameters simply insert as NULL, exactly as before this migration.
    ...
    -- otherwise this block is a complete no-op, matching prior behavior. The
    -- row was just inserted above, so there is nothing to DELETE first.
...
-- NEW 19-arg overload rather than truly replacing the old 16-arg one. Drop the
-- old overload explicitly so exactly one rpc_finalize_user_profile_full stays
-- in scope and calls with the original 16 arguments are never ambiguous.
DROP FUNCTION IF EXISTS public.rpc_finalize_user_profile_full(
  uuid, uuid, text, text, text, text, text, text, text, uuid, jsonb,
  jsonb, jsonb, jsonb, jsonb, jsonb
);

-- SECURITY-CRITICAL: this function trusts a caller-supplied p_actor_id for
-- permission/role-escalation decisions (see the header comment above and
-- 20260923010000_fix_finalize_user_profile_super_admin_escalation.sql). The
-- currently vigent 16-arg signature is granted to service_role ONLY — it is
-- explicitly REVOKEd from PUBLIC, authenticated, AND anon (20260901010000 /
-- 20260923010000), and is only ever meant to be invoked from
-- create-user/index.ts's service-role client. The new 19-arg signature MUST
-- replicate that exact restriction: granting this to `authenticated` would let
-- any authenticated caller forge p_actor_id and escalate roles/permissions.
