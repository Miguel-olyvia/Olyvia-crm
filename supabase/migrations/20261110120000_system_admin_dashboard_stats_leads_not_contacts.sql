-- get_system_admin_dashboard_stats: replace 'contacts' count with
-- qualified/negotiating leads
-- ============================================================
-- Found live while E2E-testing the contacts-to-leads merge: this RPC
-- (backing SystemAdminDashboard.tsx, the super_admin-without-active-company
-- view) still counted anew_contacts directly, showing a "Contacts" card
-- separate from "Leads" -- stale now that anew_contacts has been merged
-- into anew_leads (status qualified/negotiating). Same fix already applied
-- client-side to CompanyAdminDashboard.tsx.
--
-- JSON key renamed contacts -> qualified_leads so the frontend field name
-- reflects what it actually counts now.
...
BEGIN
  ...
  WHERE deleted_at IS NULL AND status IN ('qualified', 'negotiating');
