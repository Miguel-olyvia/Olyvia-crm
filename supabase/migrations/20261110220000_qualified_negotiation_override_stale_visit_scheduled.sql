-- get_scoped_leads_base: qualified/negotiation should win over a stale scheduled_visit_id
-- ============================================================
-- Found live: "só corrige pra que apareça o número certo da org ativa
-- sempre" -- Quick Status Cards on the Leads page (backed by
-- get_lead_status_counts -> get_scoped_leads_base) showed far fewer
-- qualified/negotiation leads than actually exist, because
-- effective_status unconditionally forces 'visit_scheduled' whenever
-- scheduled_visit_id IS NOT NULL, with no carve-out for leads that have
-- since progressed further in the pipeline.
--
-- This pre-dates the contacts-to-leads merge (client-side getEffectiveStatus
-- in AnewLeads.tsx already had a partial fix -- a STATUS_OVERRIDES_VISIT set
-- that lets 'qualified' win over a scheduled visit, but never included
-- 'negotiation', and the server-side RPC had no such carve-out at all).
-- The merge just made the gap far more visible: many migrated leads
-- legitimately have both an old scheduled_visit_id (from back when they
-- were being worked as a contact) and a real qualified/negotiation status.
--
-- Fix: 'qualified' and 'negotiation' become decisive statuses in the
-- server-side effective_status calculation too, same tier as the existing
-- 'converted' check, evaluated before the visit_scheduled checks.
