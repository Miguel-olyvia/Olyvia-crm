-- ============================================================================
-- Corrige rpc_bulk_import_products: ON CONFLICT em item_suppliers desatualizado
-- ============================================================================
-- O índice único real em item_suppliers passou de
--   item_suppliers_product_supplier_active_uniq (product_id, supplier_id)
-- para
--   item_suppliers_product_supplier_uom_active_uniq
--     (product_id, supplier_id, COALESCE(uom_id, '00000000-0000-0000-0000-000000000000'::uuid))
-- na migração 20261204202500_linhas_com_unidade_de_embalagem (aplicada à
-- produção; ainda não integrada em main). O ON CONFLICT desta função ficou
-- com o alvo antigo de 2 colunas, que já não corresponde a nenhum índice
-- único real — daí o erro "there is no unique or exclusion constraint
-- matching the ON CONFLICT specification" ao importar produtos.
--
-- Confirmado contra o esquema real antes de corrigir (pg_indexes, produção):
-- só existe hoje o índice de 3 colunas; o de 2 colunas já não existe.
--
-- rpc_bulk_import_products não define uom_id nas suas duas inserções em
-- item_suppliers, por isso COALESCE(uom_id, '00000000...') resolve sempre
-- para o mesmo sentinela — o comportamento de conflito por (produto,
-- fornecedor) mantém-se idêntico ao de antes desta correção.
-- ============================================================================

CREATE OR REPLACE FUNCTION public.rpc_bulk_import_products(p_org_id uuid, p_products jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
  ...
  FOR v_row IN SELECT * FROM jsonb_array_elements(p_products) LOOP
    ...
    BEGIN
      ...
      IF v_mode = 'insert' THEN
        ...
        --
        -- FIX (24/09): o índice único real em item_suppliers passou a ser
        -- (product_id, supplier_id, COALESCE(uom_id, sentinela)) — ver
        -- 20261204202500_linhas_com_unidade_de_embalagem. O ON CONFLICT tem
        -- de repetir exactamente essa expressão e o predicado parcial para o
        ...
        IF v_supplier_id IS NOT NULL THEN
          ...
          ON CONFLICT (product_id, supplier_id, COALESCE(uom_id, '00000000-0000-0000-0000-000000000000'::uuid))
            WHERE (deleted_at IS NULL AND product_id IS NOT NULL) DO UPDATE SET
            is_preferred = true,
            is_active    = true,
            deleted_at   = NULL,
            updated_at   = now();
      ...
      ELSIF v_mode = 'update' THEN
        ...
        -- Sincronizar item_suppliers (Fase 3) — ver comentário no topo do
        -- ficheiro / FIX acima (mesmo alvo de 3 colunas repetido aqui).
        v_supplier_id := NULLIF(v_row ->> 'supplier_id', '')::uuid;
        IF v_supplier_id IS NOT NULL THEN
          UPDATE public.item_suppliers
             SET is_preferred = false, updated_at = now()
