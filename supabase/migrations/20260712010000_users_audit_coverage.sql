-- Users Audit Coverage — Wave 2 (idempotent re-assert)
-- 2026-07-12 | Module: Utilizadores (Users) | Wave: 2 (audit trigger functions + registrations)
-- Forward-only migration. Do not fold into the baseline.
--
-- Purpose:
--   Guarantee that AFTER INSERT/UPDATE/DELETE audit coverage exists on the two
--   Users-module tables — anew_users and anew_memberships — writing to
--   entity_audit_log. This migration is idempotent: it (re)creates the dedicated
--   trigger function fn_audit_anew_users() and (re)registers both triggers via
--   DROP IF EXISTS + CREATE. It does NOT touch triggers on any other table and it
--   does NOT modify or replace the shared fn_generic_entity_audit().
--
--   It intentionally mirrors, exactly, the pattern established in:
--     20260625010000_entity_audit_log.sql   — entity_audit_log + fn_generic_entity_audit()
--     20260708010000_uom_audit_triggers.sql  — fn_audit_uom_with_sentinel()
--     20260709010000_users_audit_triggers.sql — fn_audit_anew_users() (original)
--
-- Sections:
...
--   3. REVOKE/GRANT for the dedicated function
--
-- Conventions carried over verbatim from fn_generic_entity_audit():
...
--                      org lookups would not resolve it. A dedicated function resolves the org via
...
--   A single user may belong to multiple organisations. The most relevant
...
--
-- Column baselines confirmed from 20260615130000_baseline_new_database.sql:
--   anew_users        : id, email, name, phone, avatar_url, status, created_at,
--                       updated_at, created_by, auth_user_id, description, position,
--                       location, custom_attributes, template_id, entity_id,
--                       has_completed_welcome, registration_origin, email_signature
--                       (NO organization_id column)
--   anew_memberships  : id, user_id, organization_id (NOT NULL), relationship_type,
--                       start_date, end_date, metadata, created_at, updated_at,
--                       role_id, status, invited_by, invited_at, accepted_at,
--                       join_method, created_by
...
-- does not support CREATE OR REPLACE TRIGGER). Re-asserting these does not
-- affect audit triggers on any other table.
...
--
-- 2. Confirm exactly one audit trigger per table (no duplicates):
--
