-- Utilizadores (Users) — fix missing enum cast in rpc_update_user
-- 2026-10-03 | Module: Utilizadores (Users)
-- Forward-only migration. Do not fold into the baseline. Do not edit an already-applied migration.
--
-- Problem this migration solves
-- ------------------------------
-- rpc_update_user (20260721010000_rpc_update_user_entity_ownership_fix.sql) inserts
-- pending permission-scope overrides into anew_membership_permission_scopes:
...
--      v_scope ->> 'scope_level');
-- scope_level is typed public.anew_scope_level (enum), but `v_scope ->> 'scope_level'`
-- yields plain text with no implicit cast, so every call fails with:
--   42804: column "scope_level" is of type anew_scope_level but expression is of type text
-- This blocks ALL permission-scope overrides (TEAM/OWNED/ORG) for every user via the
-- Users UI, for any organization.
--
-- Fix
-- ---
-- Cast the jsonb text value to public.anew_scope_level on insert. Every other
-- statement in this CREATE OR REPLACE is byte-identical to
-- 20260721010000_rpc_update_user_entity_ownership_fix.sql; only that single
-- VALUES expression changed.
--
-- Prerequisites:
--   20260721010000_rpc_update_user_entity_ownership_fix.sql — rpc_update_user()
