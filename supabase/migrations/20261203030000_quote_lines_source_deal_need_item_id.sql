-- quote_lines.source_deal_need_item_id — elo de volta à necessidade de origem.
-- Forward-only migration. Do not fold into the baseline. Do not edit an already-applied migration.
--
-- Why this migration exists
-- --------------------------
-- Quando um item de uma necessidade (deal_need_items) é importado para o orçamento,
-- o elo entre os dois perdia-se no momento da gravação: quote_lines não tinha onde
-- guardar a origem. Duas consequências reais:
--   (a) o botão "importar do pedido" não era idempotente — carregar duas vezes
--       duplicava linhas, porque não havia forma de saber o que já lá estava;
--   (b) depois de gravado, deixava de ser possível dizer de que necessidade veio
--       cada linha (elo partido para diagnóstico/auditoria).
-- source_deal_need_item_id fecha as duas.
--
-- DE PROPÓSITO SEM FOREIGN KEY: fn_apply_deal_need() faz delete+reinsert de
-- deal_need_items a cada gravação da necessidade, pelo que os ids de origem são
-- voláteis. Uma FK (mesmo ON DELETE SET NULL) apagaria a rastreabilidade ou faria
-- rebentar gravações de necessidades legítimas. É uma referência fraca, deliberada.
--
-- Índice parcial: a esmagadora maioria das linhas de orçamento não vem de nenhuma
-- necessidade (NULL), e só as que vêm são pesquisadas por este campo.
--
-- Parte 2 (rpc_save_quote): partiu-se da DEFINIÇÃO VIVA da função
-- (pg_get_functiondef), não de migrações antigas. Assinatura de 7 parâmetros
-- INALTERADA. Única alteração face ao vivo: source_deal_need_item_id acrescentado à
-- lista de colunas e ao VALUES do INSERT das quote_lines principais. O INSERT das
-- linhas de p_inline_quotes fica intacto — orçamentos inline são criados de raiz no
-- construtor, nunca importados de uma necessidade, por isso não têm origem a guardar.
--
-- Prerequisites:
--   20261119140000_fase1_diagnostico_orcamento_regras_e_ia_fallback.sql — quotes,
--                                              quote_lines.visible_to_client,
--                                              quote_diagnostic_area_suggestions

-- ============================================================
-- 1. quote_lines — coluna de origem
-- ============================================================
...
COMMENT ON COLUMN public.quote_lines.source_deal_need_item_id IS
  'deal_need_items.id de onde esta linha foi importada. Referência FRACA (sem FK) de propósito: fn_apply_deal_need() faz delete+reinsert dos itens da necessidade, logo os ids são voláteis. Serve para (a) idempotência do botão de importar e (b) rastrear a necessidade de origem da linha.';
...
-- ============================================================
-- 2. rpc_save_quote — gravar a origem da linha
-- ============================================================
