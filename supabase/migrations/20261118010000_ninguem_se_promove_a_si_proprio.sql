-- Ninguem se promove a si proprio.
--
-- Editar a PROPRIA ficha passa a ser sempre possivel (ver a alteracao do ecra),
-- mas cargo, ambitos, ligacoes a organizacoes e estado deixam de poder ser
-- alterados pela propria pessoa -- inclusive por administradores e system
-- admins. Quem precisa de mudar de cargo pede a outro administrador.
--
-- Porque triggers e nao RLS: rpc_update_user e SECURITY DEFINER (o RLS nao o
-- alcanca) e os ecras de Organizacoes escrevem em anew_memberships por
-- PostgREST directo. Um trigger apanha os dois caminhos. So actua quando
-- auth.uid() nao e nulo: migrations e service_role continuam livres, e e essa
-- a saida de emergencia se alguma organizacao ficar mesmo encravada.
--
-- INSERT em anew_memberships fica de fora de proposito: e por ai que a criacao
-- de uma organizacao poe o criador como org_admin dela (organizationCreation.ts
-- join_method='created_org'). Ganhar admin de uma organizacao que se acabou de
-- criar nao e promover-se dentro de uma organizacao existente.

-- Quem sou eu, do lado da base
CREATE OR REPLACE FUNCTION public.anew_current_user_ids()
RETURNS uuid[]
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  ...
  FROM (
    ...
    UNION
    SELECT m.business_user_id AS id
...
COMMENT ON FUNCTION public.anew_current_user_ids() IS
  'Ids de negocio do utilizador autenticado (anew_users + auth_to_business_user_map). SECURITY DEFINER para nao depender do RLS de anew_users. Array vazio quando nao ha auth.uid().';

GRANT EXECUTE ON FUNCTION public.anew_current_user_ids() TO authenticated;

-- anew_memberships: nao mexo na minha propria associacao
CREATE OR REPLACE FUNCTION public.tg_anew_memberships_no_self_promotion()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
...
BEGIN
  IF auth.uid() IS NULL
     OR NOT (OLD.user_id = ANY(public.anew_current_user_ids())) THEN
    IF TG_OP = 'DELETE' THEN RETURN OLD; ELSE RETURN NEW; END IF;
  END IF;

  IF TG_OP = 'DELETE' THEN
    RAISE EXCEPTION 'Nao pode remover a sua propria associacao a uma organizacao'
  ...
  IF NEW.role_id           IS DISTINCT FROM OLD.role_id
    ...
    RAISE EXCEPTION 'Nao pode alterar o seu proprio cargo, organizacao ou estado de associacao'
      USING ERRCODE = 'insufficient_privilege';
  END IF;
...
DROP TRIGGER IF EXISTS trg_anew_memberships_no_self_promotion ON public.anew_memberships;
CREATE TRIGGER trg_anew_memberships_no_self_promotion
  BEFORE UPDATE OR DELETE ON public.anew_memberships
  FOR EACH ROW EXECUTE FUNCTION public.tg_anew_memberships_no_self_promotion();

-- anew_membership_permission_scopes: nao alargo os meus proprios ambitos
CREATE OR REPLACE FUNCTION public.tg_anew_scopes_no_self_grant()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
...
BEGIN
  ...
  IF v_owner IS NOT NULL AND v_owner = ANY(public.anew_current_user_ids()) THEN
    RAISE EXCEPTION 'Nao pode alterar os seus proprios ambitos de permissao'
      USING ERRCODE = 'insufficient_privilege';
  END IF;
...
DROP TRIGGER IF EXISTS trg_anew_scopes_no_self_grant ON public.anew_membership_permission_scopes;
CREATE TRIGGER trg_anew_scopes_no_self_grant
  BEFORE INSERT OR UPDATE OR DELETE ON public.anew_membership_permission_scopes
  FOR EACH ROW EXECUTE FUNCTION public.tg_anew_scopes_no_self_grant();

-- anew_users: nao mudo o meu proprio estado
-- A politica anew_users_update (20260707030000) deixa qualquer pessoa gravar a
-- sua propria linha; sem isto, reactivar-se a si proprio era um pedido directo
-- ao PostgREST. Nenhuma outra coluna e tocada -- nome, email, telefone,
-- has_completed_welcome (WelcomeGuide) continuam livres.
CREATE OR REPLACE FUNCTION public.tg_anew_users_no_self_status_change()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
...
BEGIN
  IF auth.uid() IS NOT NULL
     ...
     AND (OLD.auth_user_id = auth.uid()
          OR OLD.id = ANY(public.anew_current_user_ids())) THEN
    RAISE EXCEPTION 'Nao pode alterar o seu proprio estado'
      USING ERRCODE = 'insufficient_privilege';
  END IF;
  RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS trg_anew_users_no_self_status_change ON public.anew_users;
CREATE TRIGGER trg_anew_users_no_self_status_change
  BEFORE UPDATE ON public.anew_users
  FOR EACH ROW EXECUTE FUNCTION public.tg_anew_users_no_self_status_change();

-- ── Parte 2: rpc_update_user ──────────────────────────────────────
-- Cópia integral do corpo de 20261112080000_rpc_update_user_system_admin_scope_bypass.sql
-- (a definição mais recente em vigor). Só duas coisas mudam: a variável
-- v_target_is_self no DECLARE e o bloco "Ninguém se promove a si próprio"
-- logo a seguir ao portão de nível 1. Tudo o resto é letra a letra igual.
--
-- Sem este bloco, quem só tem users.view e tem uma associação INACTIVA não
-- conseguia gravar a própria ficha: o ecrã só carrega as associações activas
-- para p_memberships mas envia TODAS em p_existing_membership_ids, pelo que a
-- inactiva chegava ao RPC como "apagar".
...
BEGIN
  ...
  -- ── Ninguém se promove a si próprio (20261118010000) ─────────────────
  -- Editar a própria ficha é sempre permitido (portão acima). Mudar o próprio
  -- cargo, ambitos, ligações a organizações ou estado nunca é -- nem para um
  -- administrador. Quem precisa de mudar de cargo pede a outro administrador.
  v_target_is_self := v_can_edit_self
                      OR (p_user_id = ANY(public.anew_current_user_ids()));

  IF v_target_is_self THEN
    IF COALESCE(p_status, '') IS DISTINCT FROM COALESCE(v_before_user.status, '') THEN
      RAISE EXCEPTION 'Não pode alterar o seu próprio estado'
        USING ERRCODE = 'insufficient_privilege';
    END IF;

    IF p_pending_scopes IS NOT NULL
       AND jsonb_typeof(p_pending_scopes) = 'object'
       AND p_pending_scopes <> '{}'::jsonb THEN
      RAISE EXCEPTION 'Não pode alterar os seus próprios âmbitos de permissão'
        USING ERRCODE = 'insufficient_privilege';
    END IF;

    -- Qualquer associação enviada que não seja exactamente igual à que já está
    -- gravada (cargo, organização, tipo) -- ou que seja nova -- é recusada.
    IF EXISTS (
      SELECT 1
      FROM jsonb_array_elements(COALESCE(p_memberships, '[]'::jsonb)) AS m
      WHERE NULLIF(m ->> 'organization_id', '') IS NOT NULL
        AND NOT EXISTS (
          SELECT 1
          FROM public.anew_memberships am
          WHERE am.id = NULLIF(m ->> 'id', '')::uuid
            AND am.user_id = p_user_id
            AND am.organization_id = (m ->> 'organization_id')::uuid
            AND am.role_id IS NOT DISTINCT FROM NULLIF(m ->> 'role_id', '')::uuid
            AND am.relationship_type IS NOT DISTINCT FROM NULLIF(m ->> 'relationship_type', '')
        )
    ) THEN
      RAISE EXCEPTION 'Não pode alterar o seu próprio cargo nem as suas organizações'
        USING ERRCODE = 'insufficient_privilege';
    END IF;

    -- Nada de escritas de poder no caminho da própria ficha. Assim quem só tem
    -- users.view grava os seus dados pessoais sem bater nos portões de
    -- users.edit/create/delete mais abaixo, e a lista do ecrã -- que só traz as
    -- associações activas -- nunca apaga por engano as inactivas.
    p_status                  := v_before_user.status;
    p_memberships             := '[]'::jsonb;
    p_existing_membership_ids := ARRAY[]::uuid[];
    p_pending_scopes          := NULL;
  END IF;
