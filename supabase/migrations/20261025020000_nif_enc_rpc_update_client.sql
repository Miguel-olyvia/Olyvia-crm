-- NIF Encryption — Phase 2: dual-write from rpc_update_client
...
-- get written alongside `nif` in the same UPDATE/INSERT. It changes nothing
-- about existing behavior when the 3 new trailing parameters are omitted:
-- `nif_encrypted`/`nif_hash` use COALESCE against the current row value so a
-- NULL argument never clobbers a previously-written value, and the token
-- fan-out is entirely skipped when p_nif_tokens IS NULL.
--
-- Base signature/body carried forward verbatim from the currently live
-- definition in 20260926010000_remove_root_org_id_from_rls.sql (no later
-- migration has touched rpc_update_client). Only section 5 (NIF/VAT) gains
-- the new columns/values plus the token fan-out; every other line, including
-- rpc_toggle_client_vip (untouched, not redefined here), is byte-identical.
...
-- Old 16-arg signature (through p_address_number) is superseded by this
-- 19-arg overload (3 new trailing DEFAULT NULL params) — drop it so
-- PostgREST/PostgreSQL don't have to disambiguate two overloads with the
