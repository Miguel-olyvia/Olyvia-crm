-- ============================================================
-- Persist the originating lead's marketing origin (source/source_id/
-- campaign_id) onto anew_clients when a contact is converted to a client
-- via convert_contact_to_client (supabase/migrations/20260922010000_fix_
-- convert_contact_to_client_name_corruption.sql — the current version of
-- this RPC).
--
-- Byte-identical to that version except:
--   • The initial `SELECT ... FOR UPDATE` on anew_contacts now also reads
--     c.source_lead_id, so we know which lead (if any) originally produced
--     this contact.
--   • Right after v_org_id/v_entity_id are resolved, the marketing origin is
--     resolved: first directly from anew_leads via source_lead_id when
--     present, else via the shared fn_resolve_client_marketing_origin()
--     helper (added in 20261112360000) as a fallback for contacts that
--     don't carry source_lead_id.
--   • The CREATE branch (new anew_clients row) now writes origin_source,
--     origin_source_id, origin_campaign_id with the resolved values.
--   • The REUSE branch (existing anew_clients row reactivated) now also
--     COALESCE-updates those 3 columns, never overwriting an origin already
--     recorded on the client.
-- No other logic, signature, security, or permission-check changes.
-- ============================================================
...
BEGIN
  ...
  -- Resolve marketing origin: prefer the lead that directly produced this
  -- contact (source_lead_id), else fall back to the shared resolver.
  IF v_contact.source_lead_id IS NOT NULL THEN
    SELECT source, source_id, campaign_id
    INTO v_origin_source, v_origin_source_id, v_origin_campaign_id
    FROM public.anew_leads WHERE id = v_contact.source_lead_id;
  END IF;
  IF v_origin_source IS NULL AND v_origin_source_id IS NULL AND v_origin_campaign_id IS NULL THEN
    SELECT origin_source, origin_source_id, origin_campaign_id
    INTO v_origin_source, v_origin_source_id, v_origin_campaign_id
    FROM public.fn_resolve_client_marketing_origin(v_entity_id, v_org_id);
  END IF;
  ...
  IF v_existing_client_id IS NOT NULL THEN
    ...
    SET status = 'active',
        deleted_at = NULL,
        updated_at = v_now,
        origin_source = COALESCE(origin_source, v_origin_source),
