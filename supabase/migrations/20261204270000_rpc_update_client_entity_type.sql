-- Allow editing whether an existing client is a Person or a Company.
-- Forward-only migration. Do not fold into the baseline. Do not edit an already-applied migration.
--
-- Context: the client edit form only ever READ anew_entities.type (person/organization) to
-- decide field labels/required-ness; it had no way to CHANGE it, and rpc_update_client had no
-- parameter for it. A lead created as a "Pessoa" that turns out to be a "Empresa" (or vice
-- versa) could never be corrected after conversion to client without going through support.
--
-- Currently vigent version being replaced: 20261111030000_fix_nif_plaintext_lookups_and_audit_leak.sql
-- (public.rpc_update_client(...), 20 params).
--
-- What changes vs. the vigent version
-- ------------------------------------
-- 1. One new OPTIONAL trailing parameter is appended AFTER the existing 20:
--      p_entity_type text DEFAULT NULL
--    Existing callers using the original 20-argument call (positional or named) are entirely
--    unaffected: it defaults to NULL, which is a no-op (COALESCE keeps the pre-update type).
-- 2. When provided, p_entity_type must be 'person' or 'organization' — any other value raises
--    an exception (same style as the existing permission-check RAISEs in this function).
-- 3. The anew_entities UPDATE (display_name/first_name/last_name) now also sets
--    type = COALESCE(p_entity_type, type), and the audit diff gains a 'type' entry when it
--    actually changes — same pattern already used for display_name/first_name/last_name above it.
-- 4. Adding a parameter changes the function's identity (Postgres keys a function by name +
--    argument type list), so CREATE OR REPLACE alone would leave the old 20-arg function in
--    place as a separate overload rather than replacing it — the same situation documented in
--    20261024020000_nif_enc_rpc_update_contact.sql for rpc_update_contact. The old overload is
--    dropped explicitly below so exactly one rpc_update_client stays in scope.
...
COMMENT ON FUNCTION public.rpc_update_client(
  ...
  'Server-side client update (Phase 2 NIF dual-write). p_vat empty/NULL is a no-op for the fiscal link (preserves the existing NIF) — removing it requires the explicit p_clear_nif = true signal. 20261204270000: added optional p_entity_type (person/organization) so an existing client can be reclassified between Person and Company; NULL preserves the current type.';
