/**
 * Shared address sanitization helpers (Deno).
 * Mirror of src/utils/addressSanitization.ts — keep in sync.
 */
...
export async function syncEntityPrimaryAddressFromLead(args: {
  ...
  try {
    ...
    if (!suspicious && !allowOverwriteValid) return { decision: "skip_existing_valid" };
    ...
    const activeRefs = (entRes.count ?? 0) + (orgRes.count ?? 0);

    if (activeRefs <= 1) {
      const { error: uErr } = await db.from("anew_addresses")
        .update({ ...patch, address_key: newKey })
        .eq("id", curr.id);
